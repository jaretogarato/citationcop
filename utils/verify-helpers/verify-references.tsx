...
export async function verifyReferences(
  references: Reference[],
  onProgress?: (stage: 'google' | 'openai', count: number, currentRefs: Reference[]) => void
): Promise<Reference[]> {
  ...
  for (let i = 0; i < references.length; i += GOOGLE_BATCH_SIZE) {
    ...
    try {
      ...
      onProgress?.('google', i + batch.length, []); // No refs to update during Google phase
    } catch (error) {
  ...
  for (let i = 0; i < refsToVerify.length; i += OPENAI_CONCURRENT_LIMIT) {
    ...
    try {
      ...
      verifiedRefs.push(...processedBatch);
      onProgress?.('openai', i + batch.length, verifiedRefs);
    } catch (error) {
      console.error('OpenAI batch error:', error);
      const errorBatch = batch.map(ref => ({
        ...ref,
        status: 'error' as ReferenceStatus,
        message: 'OpenAI verification failed',
        verification_source: 'Error'
      }));
      verifiedRefs.push(...errorBatch);
      onProgress?.('openai', i + batch.length, verifiedRefs);
    }
  }
  ...
  const allRefs = [...verifiedRefs, ...failedGoogleRefs];
  onProgress?.('openai', references.length, allRefs);
  return allRefs;
}
