...
export function useUrlContentVerify(): UseUrlContentVerifyResult {
  ...
  const processFailedReferences = useCallback(async (references: Reference[]): Promise<Reference[]> => {
    // Filter references that failed initial verification and have URLs
    const failedRefs = references.filter(
      ref => (ref.status === 'error' || ref.status === 'unverified') && ref.url
    );
    ...
    console.log(`Attempting URL verification for ${failedRefs.length} failed references`);

    // Process all references, but only attempt URL verification for failed ones
    const results = await Promise.all(
      references.map(async (ref) => {
        if ((ref.status === 'error' || ref.status === 'unverified') && ref.url) {
