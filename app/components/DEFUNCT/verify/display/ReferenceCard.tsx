  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <div
          className={`w-full min-w-[280px] max-w-[400px] justify-self-center bg-gradient-to-br ${getStatusColor(reference.status)} rounded-[1.5rem] overflow-hidden shadow-xl cursor-pointer hover:opacity-90 transition-opacity`}
        >
          <div className="p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div
                className={`p-2 rounded-xl cursor-pointer ${getStatusAccentColor(reference.status)} hover:opacity-80 transition-opacity ml-2`}
              >
                <Info
                  className={`h-5 w-5 ${getStatusTextColor(reference.status)}`}
                />
              </div>
            </div>

            <div
              className={`space-y-3 ${getStatusAccentColor(reference.status)} rounded-xl p-4`}
            >
              <div>
                <h3
            ...
            {reference.message && (
              <div
                className={`${getStatusAccentColor(reference.status)} rounded-xl p-4`}
              >
                <h3
                  className={`text-sm font-medium ${getStatusTextColor(reference.status)} mb-1`}
                >
                  Verification Notes
                </h3>
